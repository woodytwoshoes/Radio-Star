...
def show_image_and_feats(idx):
    ...
    print(pd.DataFrame(raw_df.loc[idx].drop('Path')).T.replace(
        to_replace=encoded_dict))
    plt.show()


def show_image_and_complement(idx, complementary_feat):
    row = raw_df.loc[idx]
    idx_complement = (raw_df[raw_df[complementary_feat] == 0 - row[
        complementary_feat]] == row).sum(axis=1).idxmax()
    ...
    ax[0].set_title(str(encoded_dict[row[complementary_feat]]) + str(
        complementary_feat) + ' ' + 'Pt ID: ' + str(idx) + '\n')

    ax[1].imshow(img_B, cmap='bone')
    ax[1].set_title(str(encoded_dict[0 - row[complementary_feat]]) + str(
        complementary_feat) + ' ' + 'Pt ID: ' + str(idx_complement) + '\n')
